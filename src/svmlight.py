...
class SVMlight(BaseEstimator, ClassifierMixin):
    ...
    This implementation was originally provided by the public repository at this link: https://github.com/AlexMoreo/pydci/blob/master/src/classification/svmlight.py
    :Author: Alejandro Moreo
    """

    valid_kernels = {"linear": 0, "rbf": 2}

    def __init__(self, svmlightbase="./svm_light", C=None, kernel="linear", transduction=None, verbose=False):
        """
        ...
        """
        assert kernel in self.valid_kernels, "unsupported kernel {}, valid ones are {}".format(
            kernel, list(self.valid_kernels.keys())
        )
        self.tmpdir = None
    ...
    def fit(self, X, y):
        ...
        if self.C is not None:
            options += ["-c " + str(self.C)]
        if self.kernel is not None:
            options += ["-t " + str(self.valid_kernels[self.kernel])]
        if self.is_transductive:
            options += ["-l " + transductions_path]
        ...
        if self.transduction is not None:
            self.transduced_labels = np.loadtxt(
                transductions_path,
                usecols=0,
    ...
    def predict(self, X, y=None):
        ...
        )  # this would allow to run parallel instances of predict hopefully
    ...
    def predict_proba(self, X, y=None):
        """
        Simple wrapper of SVMlight.predict(...) that transforms the predicted
        labels in a one-hot matrix.
